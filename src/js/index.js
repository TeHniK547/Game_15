...
const winFlagArray = new Array(16).fill(0).map((_item, i) => i + 1);
function isWin(matrix) {
    const flagMatrix = matrix.flat();
    for(let i = 0; i < winFlagArray.length; i++) {
        if (flagMatrix[i] !== winFlagArray[i]) {
            return false;
        }
    }
